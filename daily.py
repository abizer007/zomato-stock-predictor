...
def aggregate_predictions(predictions, time_period):
    ...
    df_pred = pd.DataFrame(
        predictions, columns=[f"Prediction_{i+1}" for i in range(predictions.shape[1])]
    )
    ...
    if time_period == "monthly":
        aggregated_predictions = df_pred.resample(
            "M"
        ).sum()  # Summing the daily predictions to get monthly predictions
    elif time_period == "yearly":
        aggregated_predictions = df_pred.resample(
            "Y"
        ).sum()  # Summing the daily predictions to get yearly predictions
    else:
...
try:
    with open("monthly_predictions.txt", "a") as pred_file:
        pred_file.write(
            f"Monthly predictions on {datetime.now()}:\n{monthly_predictions}\n"
        )
...
try:
    with open("yearly_predictions.txt", "a") as pred_file:
        pred_file.write(
            f"Yearly predictions on {datetime.now()}:\n{yearly_predictions}\n"
        )
...
try:
    with open("stock_data.csv", mode="a", newline="") as file:
        ...
        writer.writerow(
            [datetime.now().strftime("%Y-%m-%d"), "1000", "Prediction Value"]
...
files_to_check = [
    "performance.txt",
    "weekly_predictions.txt",
    "monthly_predictions.txt",
    "yearly_predictions.txt",
    "stock_data.csv",
]
